...
The cheapest Telegram numbers start around **$0.05** per attempt, varying by country. See [current pricing](https://virtualsms.io/pricing) for live rates. If the SMS doesn't arrive, `cancel_order` refunds the full cost (provided you waited the 2-minute minimum, which `wait_for_code` enforces internally).
