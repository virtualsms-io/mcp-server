...
- **Public tools** (`get_price`, `find_cheapest`, `check_number`, `list_proxy_locations`) read public catalog data and are safe to call from public agents without a key.
- **Everything else**, including `list_services`, `list_countries` and `search_services`, reads account-scoped endpoints and requires a valid API key. Without one, the server returns a structured error and never falls back to anonymous behavior.
